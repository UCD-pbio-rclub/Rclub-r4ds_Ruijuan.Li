...
# gampminder 
# Hans Rosling 
library("gapminder") # not availble ... 
gapminder
...
glance <- by_country %>% 
  mutate(glance = map(model, broom::glance)) %>% 
  unnest(glance, .drop = TRUE) # drop unrelated columns 
glance
...
test <-
gapminder %>% 
  nest(year:gdpPercap) # nesting on ungruped data 
...
df <- tribble(
  ...
  list(a = 1, b = 2),
  list(a = 2, c = 4)
) # x column with two lists, each have two items within 
df
df %>% mutate(
  a = map_dbl(x, "a"),
